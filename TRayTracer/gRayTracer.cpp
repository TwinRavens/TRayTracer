...
GLint rav::RayTracer::Setup(int width, int height, int depth)
{
	...
	#pragma region Passing Buffers to Shader
	{
		...
		#pragma endregion
		objData = RayFileLoader::LoadObject("./data/mesh/test4.obj");
		...
		#pragma region Vertex
		{
			...
			glm::vec4* vertex = objData->vertices;
			//Copy data to OpenGL
			GLuint ssbo = 0;
			glGenBuffers(1, &ssbo);
			glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo);
			glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(*vertex)  * objData->verticesCount, vertex, GL_STATIC_DRAW);
		...
		#pragma region Normals
		{
			...
			glm::vec4* normals = objData->normal;

			//Copy data to OpenGL
			GLuint ssbo = 0;
			glGenBuffers(1, &ssbo);
			glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo);
			glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(*normals) * objData->verticesCount, normals, GL_STATIC_DRAW);
		...
		#pragma region Triangles
		{
			...
			{
				//Triangle triangles[] = {
				//	//	v1	v2	v3	p1	n1	n2	n3	p2
				//	{	0,	1,	2,	1,	0,	0,	0,	1	},	//Triangle 1
				//	{	3,	4,	5,	1,	0,	0,	0,	1	}	//Triangle 2
				//};
			}

			Triangle *triangles = objData->triangles;
			//Copy data to OpenGL
			GLuint ssbo = 0;
			glGenBuffers(1, &ssbo);
			glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo);
			glBufferData(GL_SHADER_STORAGE_BUFFER, sizeof(*triangles)* objData->triangleCount, triangles, GL_STATIC_DRAW);
